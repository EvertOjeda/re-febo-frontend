...
const DevExpressList = React.memo((props) => {
  ...
  Main.useHotkeys(Main.nuevo, (e) =>{
    ...
    const columnModal     = props.arrayColumnModal; const defaultFocusColumn = props.defaultFocusColumn; const maxFocus = props.maxFocus;
  ...
  const onFocusedCellChanging = async(e)=> {    
    ...
    if(e.event){
      if(maxFocus){
        if( (e.event.key === "Enter" || e.event.code == "ArrowRight" || e.event.key === "Tab") && e.columns[e.prevColumnIndex].name === maxFocus[0].hasta){
          let rowindex = e.prevColumnIndex == - 1 ? 0 : e.prevColumnIndex
          e.cancel = true
          grid.current.instance.focus(grid.current.instance.getCellElement(e.prevRowIndex,rowindex))
        }
      }
    }
  }
      ...
      try{
        ...
        .then(async response => {
          if(response.status == 200 ){
            ...
            if(maxFocus){
              if(Object.keys(rowsData.newData)[0] == maxFocus[0].hasta && maxFocus[0].id == FormName){
                rowsData.cancel = true
                let rowIndex    = grid.current.instance.getRowIndexByKey(rowsData.oldData.ID) == -1 ? 0 : grid.current.instance.getRowIndexByKey(rowsData.oldData.ID);
                grid.current.instance.focus(grid.current.instance.getCellElement(rowIndex,Object.keys(rowsData.newData)[0]))
              }
            }
  ...
  const funcionCancelar = async ()=>{
    if(dataAuxCancel == "") dataAuxCancel = "[]" 
