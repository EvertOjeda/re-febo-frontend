...
const NewTableSearch = (props) => {
    ...
    const keyDownInput = async(e)=>{
        if(e.keyCode === 13){
            ...
            if(keyColumn.length == 3){
                modalSetOnClick(getDataRows(), tipoDeBusqueda, keyColumn);
            }else{
                let keyColumn = Object.keys(getDataRowsKey());
                let keyValue  = Object.values(getDataRowsKey());
                modalSetOnClick(keyValue, tipoDeBusqueda, keyColumn);
            }            
        }
        var key = e.which;
        if(key == '27'){
