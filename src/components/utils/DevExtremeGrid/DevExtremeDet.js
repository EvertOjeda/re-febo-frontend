...
import {setComponenteEliminar,
        setTipoDeOperacion   ,
        getTipoDeOperaciones } from './DevExtremeCab';
...
var componenteFocusDet = {};
export const setComponenteFocusDet = (e)=>{
  componenteFocusDet = e;
};
export const getComponenteFocusDet = ()=>{
  return componenteFocusDet;
}
...
const DevExtremeDet =  ({   gridDet   , initialRow         , notOrderByAccion     ,
                            ...
                            limpiarColumnaRequerida = false, newButtonCancelar
                        }) => {
    ...
    useEffect(()=>{      
      // columnEditDisable = []
      if(limpiarColumnaRequerida) columnRequerido = []
    ...
    const onFocusedCellChanging = async(e) =>{
      ...
      if(e.rows[fila]){
        ...
        var valor = getComponenteFocusDet()
        if(Object.keys(valor).length !== 0){
          valor[id] = _.union(valor,[{'rowIndex':fila, 'columnIndex':e.newColumnIndex}])
        }else{
          valor[id] = {'rowIndex':fila, 'columnIndex':e.newColumnIndex}
        }
        setComponenteFocusDet(valor)
      }
      ...
      if(e.event){
          ...
          if(totalRows === fila && rowNew && getbandBloqueoGrid()){
            ...
            if(columnRequerido.length){
              ...
              for (let index = 0; index < columnRequerido.length; index++) {
                  ...
                  let idComponente = getComponenteEliminarDet()
                  if(element.IDCOMPONENTE == idComponente.id){
        ...
        // maxFocus[0].hasta !== e.columns[e.newColumnIndex].dataField
        if ((e.event.key === "Enter" || e.event.key === "Tab") && !e.event.shiftKey && maxFocus == undefined) {
    ...
    const addRow = async() => {
        ...
        var rowIndex = 0
        rowDet       = gridDet.current.instance.getDataSource();        
        if(newButtonCancelar) Main.modifico(FormName); else modifico();
    ...
    const onRowUpdating  = async(value) =>{
      ...
      if(columnModal){
        ...
        }else if(columnModal.urlValidar.length > 0){
          if(columnModal.urlValidar[0][columnNameEdit]){
            if(!activarBandF9){
              await funcionValidacion(columnNameEdit[0], value.newData[columnNameEdit[0]], value);
            }else{
              activarBandF9 = false
      ...
      if(operacion){
        switch (operacion.operac) {
          ...
          break;
        case 'suma':
          if(operacion.IDCOMPONENTE == id && operacion.idSuma.includes(columnNameEdit[0])){
            let totalSuma = 0;
            operacion.idSuma.forEach(element => {
              if(element == columnNameEdit[0]){
                totalSuma = totalSuma + value.newData[element]
              }else{
                let valorAux = _.isUndefined(value.oldData[element]) ? 0 : value.oldData[element]
                totalSuma = totalSuma + valorAux
              }
            });
            value.oldData[operacion.resultado] = totalSuma;
          }
        default:
          break;
      }
      ...
      if(columnModal.config[nameData]?.depende_ex_cab !== undefined){
        for (let i = 0; i < columnModal.config[nameData].depende_ex_cab.length; i++){
          ...
          var dataName = items.id
            if(dataCabecera[dataName]?.length == 0){

              console.log('Valor ',dataCabecera[dataName], 'Row =>',dataCabecera, 'name ',dataName);
    ...
    const modalSetOnClick = async (datos, busquedaPor, datoskey) => {
      ...
      if(datos !== "" || datos !== undefined){
        ...
        var indexColum         =  0
        if(columnModal[busquedaPor]){
          if(columnModal[busquedaPor][0].multiple_header){
            var desPropiedadColumn = await gridDet.current.instance.columnOption(columnModal[busquedaPor][1].ID)
            indexColum             = codPropiedadColumn.index - 1;
            if(indexColum == -1 ) indexColum = 0
          }else{
            indexColum = codPropiedadColumn.visibleIndex
          }
        }else{
          indexColum = codPropiedadColumn.visibleIndex
        };
            ...
            if(!datoskey.length){
              ...
              for (let i = 0; i < datoskey.length; i++) {
                ...
                if(!band){
                  ...
                  if(element != 'ID'){
                    getFocusGlobalEventDet().row.data[element] = datos[i]
                  }  
                }
              }   
            }
        ...
        if(!e.event.repeat){
          ...
          if ((e.event.key === "Enter" || e.event.key == "Tab") && !e.event.shiftKey) {
            if(maxFocus){
              ...
              let visiblerowPrueba = visiblerow.length -1 == -1 ? 0 : visiblerow.length -1

              if(maxFocus[0].id === id && visiblerowPrueba == indexRow ){
       ...
       if(e.rowType !== "header"){ 
        ...
        if(e.row !== undefined){
          ...
          if(e.columnIndex){
            ...
            var valor = getComponenteFocusDet()
            if(Object.keys(valor).length !== 0){
              valor[id] = _.union(valor[id],{'rowIndex':e.rowIndex, 'columnIndex':e.columnIndex})
            }else{
              valor[id] = {'rowIndex':e.rowIndex, 'columnIndex':e.columnIndex}
            }
            setComponenteFocusDet(valor);
          } 
          if(e.rowIndex){
