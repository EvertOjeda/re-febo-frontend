...
const Chart = () => {
    ...
    const verify = () => {
        ...
        prueba = prueba.filter( (item)=>{
            return  item.NOM_FORMA == 'BSHOME' && 
                    item.PARAMETRO == 'DH_WIDGET_FACTURACION' &&
                    item.PERMISO   == 'S'
        });
        if(prueba.length > 0){
            setShow(true);
    ...
    const getData = async() =>{
		...
		var url   = `/widget/facturaciones/` + sessionStorage.getItem('cod_empresa') + '/' + fecha;
                ...
                ?   <Col span={6}>
                        <Spin spinning={loading}>
                            <div className="widget-dashboard-card">
                                <Title level={5} className="widget-title">Facturación</Title>
                                <Column {...config} />
