...
const columnsDet = [
    { ID: 'COD_TIPO'     , label: 'Tipo'         , width: 80      , align:'left'   , editModal:true , requerido:true  , upper:true},
...
const MarcasList = React.memo( () => {
	...
	const guardar = async ()=>{
		...
		if(valor) return;
		// CABECERA
		var contentCab   = await Main.GeneraUpdateInsertCab(rows, 'COD_MARCA', url_cod_marca,[],true);
