...
const columns = [
  ...
  { ID: 'TITULO_1'      , label: 'Depósitos' , align:'center' , multiple_header:true,
    multiple_column:[
...
const columnModal = {
  ...
  COD_DEPOSITO:[    //add multiple_header:true a todos los que se encuentren dentro del encabezado
    { ID: 'COD_DEPOSITO' , label: 'Deposito'   , width: 110  , align:'left', multiple_header:true}, 
    { ID: 'DESC_DEPOSITO', label: 'Descripción', minWidth: 70, align:'left'}
  ],
  COD_DEPOSITO_ENT:[
    { ID: 'COD_DEPOSITO_ENT'  ,label: 'Deposito'     , width: 110      , align:'left' , multiple_header:true},
    { ID: 'DESC_DEPOSITO_ENT' ,label: 'Descripción ' , minWidth: 70    , align:'left'},
  ],
  COD_UNIDAD_MEDIDA:[
    { ID:'COD_UNIDAD_MEDIDA', label: 'Unidad de Medida', width: 110, align:'left',multiple_header:true},
    { ID:'DESC_UNIDAD_MEDIDA', label: 'Descripción', minWidth: 70, align:'left'},
  ],
  COD_CAUSA:[
    { ID:'COD_CAUSA', label: 'Causa', width: 110, align:'left',multiple_header:true},
...
const STENVIO = memo(() => {
  ...
  useEffect(() => {
    document.getElementById("form-div-stenvio").addEventListener('click', function (e){
			bandCabDet = true
		});
    document.getElementById("form-div-stenvio-grid").addEventListener('click', function (e){
			bandCabDet = false
		});
    clearForm();
    setTimeout( ()=>{
  ...
  const getCambioDolares = async() => {
    try {
			return await Main.Request(url_tipo_cambio, "POST", { p_cod_moneda:2, p_tipo:'D', p_fecha:Main.moment().format('DD/MM/YYYY'), p_opcion:'V'});
		} catch (error) {
			console.log(error);
		} finally {
			setActivarSpinner(false);
  ...
  const clearForm = async(limp_D_Cab = true) =>{
    ...
    form.setFieldsValue({
      ...
      ['NRO_ENT_SAL'  ]: '',
      ['TIP_CAMBIO_US']: tipo_cambio.data,
      ['TIP_CAMBIO_US_FORMAT']: tipo_cambio.data ? currency(tipo_cambio.data, { separator:'.',decimal:',',precision:0,symbol:'' } ).format() : '' ,
      ['COD_EMPRESA'  ]: sessionStorage.getItem('cod_empresa'),
      ['COD_USUARIO'  ]: sessionStorage.getItem('cod_usuario'),
      ['COD_SUCURSAL' ]:'',
      ['COD_MOTIVO'   ]:'',
  ...
  const SaveForm = async() => {
    ...
    var delete_cabecera = _.isUndefined(DeleteCabecera['CABECERA']) ? [] : DeleteCabecera['CABECERA'];
    ...
    if(!DeleteCabecera['CABECERA']){
      ...
      if(lineas.length == 0 && delete_detalle.length == 0){
        Main.message.info({
        ...
        try {
          ...
          .then( response => {
            if(response.data.ret == 1){
              ...
              if(DeleteCabecera['CABECERA']){
                let valor = rows.filter((items)=>{
                  return items.NRO_COMPROBANTE !== DeleteCabecera['CABECERA'][0].NRO_COMPROBANTE;
                });
  ...
  const ManejaF7 = async(focus = false) =>{
    if(!getBloqueoCabecera()){
      //  F7
      EstadoFormulario('I');
      var tipo_cambio = await getCambioDolares();
      ...
      setIndice(0);
      if(focus)setTimeout(()=>document.getElementById('NRO_COMPROBANTE').focus(),300);
    }else{
      setShowMessageButton(true)
      showModalMensaje('Atencion!','atencion','Hay cambios pendientes. ¿Desea guardar los cambios?');
    }
