...
const Titulo            = "Zonas Geográficas";
...
const columns = [
	{ ID: 'COD_ZONA'	        , label: 'Código'   	, width: 100    },
	{ ID: 'DESCRIPCION'   		, label: 'Descripción' 	, minWidth: 200 },
  	{ ID: 'BARRIOS'   		    , label: 'Barrio'       , minWidth: 100 },
];
const defaultOpenKeys     = sessionStorage.getItem("mode") == 'vertical' ? [] : ['BS','BS-BS1'];
const defaultSelectedKeys = sessionStorage.getItem("mode") == 'vertical' ? [] : ['BS-BS1-null-BSZONAS'];
const FormName            = 'BSZONAS';
export default () =>{
    const url_base      = "/bs/zonas_geograficas/"        + sessionStorage.getItem('cod_empresa');
    const urlBuscador   = "/bs/zonas_geograficas/search/" + sessionStorage.getItem('cod_empresa');
