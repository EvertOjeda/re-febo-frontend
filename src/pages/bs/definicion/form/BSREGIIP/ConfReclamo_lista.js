...
const Titulo            = "Configuracion de Reclamo";
const url_lista         = '/bs/confreclamo/';
const columBuscador     = 'COD_USUARIO';
const doNotsearch       = ['ACCION'];
const notOrderByAccion  = ['ACCION'];
const idACCION          = 'ACCIONES'; 
const columns = [
    { ID: "COD_USUARIO"    , label: "Usuario"       , width:100 },
    { ID: "DESC_USUARIO"   , label: "Nombre"        , minWidth: 300 , align: "left" , alignID: "left" },
    { ID: "ESTADO"         , label: "Estado"        , minWidth: 100 , width:100     , align: "center", alignID: "right" , checkbox:true, options:['S']}
];
const defaultOpenKeys     = sessionStorage.getItem("mode") == 'vertical' ? [] : ['BS','BS-BS1'];
const defaultSelectedKeys = sessionStorage.getItem("mode") == 'vertical' ? [] : ['BS-BS1-null-BSREGIIP'];
const FormName            = 'BSREGIIP';
export default () => {

    const urlBuscador   = "/bs/confreclamos/search/" + sessionStorage.getItem('cod_empresa');
    const url_base      = "/bs/confreclamos/" + sessionStorage.getItem('cod_empresa');
