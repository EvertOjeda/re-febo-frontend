...
const Titulo            = "Tipos de cambio por Moneda";
const url_lista         = '/bs/tipcambio/';
const urlBuscador       = "/bs/tipcambio/search";
const columBuscador     = 'COD_MONEDA';
const doNotsearch       = ['ACCION'];
const notOrderByAccion  = ['ACCION'];
const idACCION          = 'ACCIONES';
const columns = [
    { ID: 'COD_MONEDA'                , label: 'Código'      , width: 100    },
    { ID: 'DESC_MONEDA'               , label: 'Descripción' , minWidth: 50  },
    { ID: 'FEC_TIPO_CAMBIO'           , label: 'Fecha'       , minWidth: 130  , align:'left' },
    { ID: 'VAL_VENTA'                 , label: 'Venta'       , minWidth: 110  , align:'right' , format: 'number'},
    { ID: 'VAL_COMPRA'                , label: 'Compra'      , minWidth: 90   , align:'center', format: 'number'},
];
const defaultOpenKeys     = localStorage.getItem("mode") == 'vertical' ? [] : ['BS','BS-BS1'];
const defaultSelectedKeys = localStorage.getItem("mode") == 'vertical' ? [] : ['BS-BS1-null-BSTIPCAM'];
const FormName            = 'BSTIPCAM';
export default () => {
    const url_base      = "/bs/tipcambio/"
