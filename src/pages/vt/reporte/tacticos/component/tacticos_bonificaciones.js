...
const Reporte = ()=>{
    ...
    const getData = async() =>{
        ...
        await Main.Request( url, 'POST',{tipo:'bonificacion', cod_empresa: sessionStorage.getItem('cod_empresa')})
            .then( response =>{
    ...
    const getProducts = async(info, value) => {
        ...
        len = len.length;
        var bon = await getBonificaciones(info, "COD_ARTICULO_A_BON" );
        var bonificacion = '';
        if(content.length > 1){
            for (let index = 0; index < content.length; index++) {
                const item = content[index];
                bonificacion = '<td></td><td></td><td></td><td></td><td></td>';
                if(index < bon.length ){
                    bonificacion = bon[index];
                }
                if(index > 0){
                    html += '<tr>';
                }
    ...
    const getBonificaciones = (info, value ) => {
        var rows = [];
        var html = '';
        var content = _.uniq(info, function(item){
            return item[value];
        ...
        for (let index = 0; index < content.length; index++) {
            const item = content[index];
            var html = '';
            html += '<td>' + item.COD_ARTICULO_A_BON + '</td>';
            html += '<td>' + item.DESC_ARTICULO_A_BON + '</td>';
            html += '<td>' + item.COD_UNID_MEDIDA_A_BON + ' - ' +  item.REFERENCIA_A_BON + '</td>';
            html += '<td class="right aligned">' + item.CANTIDAD_A_BON + '</td>';
            html += '<td class="center aligned">' + item.IND_MULTIPLE + '</td>';
            rows.push(html);
        }
        return rows;
    }
    ...
    const Build = async(rows_data) =>{
        ...
        $('#table-body-bonificacion').empty();
        $('#table-body-bonificacion').append(html);
    }
    ...
    const export2PDF = async() =>{
        ...
        pdfDoc.autoTable({
            ...
            columnStyles:{
                ...
                // Descripcion
                15: {halign: 'left', cellWidth:45},
                // Unidad de medida
                16: {halign: 'left', cellWidth:40},
                // Cantidad
                17: {halign: 'right', cellWidth:30},
                // Codigo
                18: {halign: 'left', cellWidth:25},
                // Descripcion
                19: {halign: 'left', cellWidth:45},
                // Unidad de medida
                20: {halign: 'left', cellWidth:40},
                // Cantidad
                21: {halign: 'right', cellWidth:30},
                // Multiplo
                22: {halign: 'center', cellWidth:30},
            },
            ...
            didDrawPage: function (data) {
                ...
                pdfDoc.text('Tácticos - Bonificaciones',(pdfDoc.internal.pageSize.getWidth() / 2) - 70,30);
        ...
        pdfDoc.save('tacticos_bonificaciones_' + moment().format('DD_MM_YYYY') + '.pdf');
    }

    const export2Excel = async() =>{
        setActivarSpinner(true);
        createHtml2ExportTable2Excel2(data,'Tactico_de_bonificacion_' + moment().format('DD_MM_YYYY') );
        setTimeout( ()=> {
            setActivarSpinner(false);
        }, 200);
    }
    ...
    return(
        <Spin spinning={activarSpinner}>
            ...
            <table 
                className="ui basic compact celled table" 
                style={{fontSize: '.6em', marginBottom:'15px', marginTop:'5px'}}
                id="table-bonificacion">
                <thead className="headTable">
                    <tr>
                        ...
                        <th rowSpan="2" width="500">Perfil</th>
                        <th rowSpan="2" width="200">Lista de precios</th>
                        <th rowSpan="2" width="150">Jefe Venta</th>
                        <th rowSpan="2" width="150">Supervisor</th>
                        <th rowSpan="2" width="150">Vendedor</th>
                        <th colSpan="4">Productos Causales</th>
                        <th colSpan="5">Bonificación</th>
                    </tr>
                    <tr>
                        ...
                        <th width="50">Unidad</th>
                        <th width="50">Cantidad</th>
                        <th width="50">Multiplo</th>
                    </tr>
                </thead>
                <tbody id="table-body-bonificacion" className="table-prueba"></tbody>
            </table>  
        </Spin>           
    )
}
