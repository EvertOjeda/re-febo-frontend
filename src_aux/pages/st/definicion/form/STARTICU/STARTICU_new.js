...
import { Row, Col, Checkbox, Radio, Form, Input, Select, Tabs, DatePicker, ConfigProvider, Typography, Card, Button, Divider  } from "antd";
...
import guardarIcono from '../../../../../assets/icons/diskette.svg';
...
// }
// VALIDA
const url_valida_articulo_ref = "/st/starticu_new/valida/articulo";
const url_valida_pais = "/st/starticu_new/valida/pais";
const url_valida_proveedor_dflt = "/st/starticu_new/valida/proveedor";
...
// BUSCA
const url_buscar_articulo_ref = '/st/starticu_new/buscar/articulo';
...
const ColumnProveedor = [
	{ ID: 'COD_PROVEEDOR_DFLT', label: 'Código', width: 50 },
...
var DataAux = "";
var DataAuxUnidadMedida = "";
const data_len = 10;

const url_cabecera = '/st/starticu_new';
const url_cod_articulo = '/st/starticu_new/cod_articulo';
// const url_valida_articulo_ref

var ValidaInput = [
  {
    input: 'COD_ORIGEN_ART',
    url: url_valida_articulo_ref,
    url_buscar: url_buscar_articulo_ref,
    valor_ant: null,
    out:[
      'DESC_ORIGEN_ART'
    ],
  ...
  },
  {
    ...
    url_buscar: url_valida_proveedor_dflt,
    valor_ant: null,
    out:[
      'DESC_PROVEEDOR'
    ],
...
const Articulo = memo(() => {
  ...
  const [ Data, setData ] = useState([]);
    ...
    var valor = {
      ...
      // ['TIP_CAMBIO_US']: tipo_cambio.data.rows[0]?.VAL_VENTA ? new Intl.NumberFormat("de-DE").format( tipo_cambio.data.rows[0].VAL_VENTA.toFixed(0) ) : '' ,
      // ['TIP_COMPROBANTE']: 'ENV',
      // ['SER_COMPROBANTE']: 'A',
      // ['NRO_COMPROBANTE']: '',
      // ['ESTADO']: 'P',
    ...
    if(e.keyCode == 13 || e.keyCode == 9){
      ...
      if(e.target.id == 'DESCRIPCION') document.getElementById('COD_ORIGEN_ART').focus();
      ...
      if(e.target.id == 'NRO_REGISTRO') document.getElementById('INSCRIPCION').focus();
      if(e.target.id == 'INSCRIPCION') document.getElementById('COD_NOMEN').focus();
			if(e.target.id == 'COD_NOMEN') document.getElementById('COD_IVA').focus();
			if(e.target.id == 'CANT_PED') document.getElementById('CANT_BASE').focus();
			if(e.target.id == 'CANT_BASE') document.getElementById('CANT_VOLUMNE').focus();
			if(e.target.id == 'CANT_VOLUMNE') document.getElementById('COD_CAT_ALM').focus();
			if(e.target.id == 'COD_ALTERNO') document.getElementById('NRO_ORDEN').focus();
			if(e.target.id == 'NRO_ORDEN') GridUnidadMedida.current.instance.focus(GridUnidadMedida.current.instance.getCellElement(0,0));
			if(	
    ...
    if(e.keyCode == '120'){
      ...
      switch(e.target.id){
				case "COD_ORIGEN_ART":
          aux = await getData( {cod_empresa: sessionStorage.getItem('cod_empresa')}, url_buscar_articulo_ref );
        ...
        case "COD_PAIS_ORIGEN":
          ...
          break;
				case "COD_PROVEEDOR_DFLT":
					aux = await getData( {cod_empresa: sessionStorage.getItem('cod_empresa')}, url_buscar_proveedor);
				...
				case "COD_RUBRO":
					...
					break;
				case "COD_FAMILIA":
					aux = await getData( {cod_empresa: sessionStorage.getItem('cod_empresa')}, url_buscar_familia);
    ...
    if(!getBloqueoCabecera()){
      ...
      setData([{
        ...
        // ['SER_COMPROBANTE']: 'A',
        // ['NRO_COMPROBANTE']: '',
        // ['TIP_CAMBIO_US']: tipo_cambio.data.rows[0]?.VAL_VENTA ? tipo_cambio.data.rows[0].VAL_VENTA : '' ,
        ['insertedDefault']: true,
      }])
      setIndice(0);
    }else{
	...
	const manageTabs = async(value) =>{
		switch (value) {
			case "1":
				if(band_unidad_medida){
					...
					if(info.data.rows.length == 0){
						...
						content = [{
							...
							IND_VENTA     : "N",
							MEN_UN_VTA    : "N",
							idCabecera    : form.getFieldValue('ID'),
							InsertDefault : true,
							IDCOMPONENTE  : "UNIDAD_MEDIDA",
						}]
					}else{
						content = info.data.rows;
						DataAuxUnidadMedida = JSON.stringify(info.data.rows);
					}
				...
				// LEAD TIME
				let content2 = [];
				let info2 = await Main.Request('/st/starticu/lead_time','POST',{
					cod_empresa: sessionStorage.getItem("cod_empresa"),
					cod_articulo: form.getFieldValue('COD_ARTICULO')
				});
				if(info2.data.rows.length == 0){
					var newKey = uuidID();
					content2 = [{
						ID	         : newKey,
						COD_ARTICULO : form.getFieldValue('COD_ARTICULO'),
  ...
  //       },4);
    ...
    for (let index = 0; index < update_insert_cabecera.length; index++) {
      ...
      try {
        if(element.inserted){
          let info = await Main.Request( url_cod_articulo, 'POST', {'cod_empresa':sessionStorage.getItem('cod_articulo')} )
          element.COD_ARTICULO = info.data.rows[0].ID;
        }
      } catch (error) {
		...
		var infoUnidadMedida = await Main.GeneraUpdateInsertDet(UnidadMedidaData,['COD_ARTICULO'], update_insert_cabecera, [],"COD_ARTICULO");
    var aux_unidad_medida = infoUnidadMedida.rowsAux;
    var update_insert_unidad_medida = infoUnidadMedida.updateInsert;

		// console.log('infoUnidadMedida => ', infoUnidadMedida);
  ...
  // UNIDAD DE MEDIDA
	...
	// INFO
		...
		var data = {
			...
			aux_update_insert_unidad_medida: JSON.parse(DataAuxUnidadMedida),
                ...
                <Main.Spin size="large" spinning={activarSpinner}>
									<div className="paper-container">
										...
										<div className="paper-header-menu">
											...
											/>
											<Button
												icon={<img src={guardarIcono} width="20" style={{ marginBottom: '3px', }} />}
												className="paper-header-menu-button"
												onClick={guardar}
												/>
											<Button 
									...
									<Form size="small" form={form} style={{marginTop:'10px', paddingLeft:'20px', paddingRight:'20px'}}>
										<Col span={24}>
											<Row gutter={16}>
												<Col span={9}>
													<Form.Item label={<label style={{width:'80px'}}><span style={{color:'red'}}>*</span> Codigo</label>}>
														<Form.Item name="COD_ARTICULO" style={{width:'80px',  display:'inline-block', marginRight:'4px'}} required >
															<Input type="number" className="search_input requerido" onChange={handleInputChange} onKeyDown={handleKeyDown} onKeyUp={handleKeyUp} />
														</Form.Item>
														<Form.Item name="DESCRIPCION" style={{width:'calc(100% - 84px)', display:'inline-block'}}>
															<Input onChange={handleInputChange} onKeyDown={handleKeyDown} onKeyUp={handleKeyUp} />
														</Form.Item>
													</Form.Item>
													...
													<Form.Item label={<label style={{width:'80px'}}>Pais/Origen</label>}>
														<Form.Item name="COD_PAIS_ORIGEN" style={{width:'80px',  display:'inline-block', marginRight:'4px'}}>
															<Input type="text" className="search_input requerido" 
												...
												<Col span={10}>
													...
													<Row>
														...
														<Col span={12}>
															<Form.Item label={<label style={{width:'90px'}}>Reg/Turismo</label>}>
																<Checkbox></Checkbox>
															</Form.Item>
														</Col>
													</Row>
													...
													<Row>
													  ...
													  <Form.Item
															label={<label style={{width:'100px'}}>Med. Compra</label>}
															name="IND_MED_COMP"
															// labelCol={{ span: 5 }}
															// wrapperCol={{ span: 19 }}
															// onChange={async(e)=>{
															...
															// }}
														>
															<Card style={{paddingTop:'5px', background:'green'}} >
																<Radio.Group>
																	<Radio value="N">
																		Ning.
																	</Radio>
														...
														<Form.Item
															// label="Mes"
															label={<label style={{width:'100px'}}>Mes</label>}
															name="IND_MES"
															// labelCol={{ span: 5 }}
															// wrapperCol={{ span: 20 }}
															// onChange={async(e)=>{
													  ...
													  <Row>
															<Col span={8}>
																<Form.Item name="CANT_PED" label={<label style={{width:'100px'}} >Cantidad</label>}>
																	<Input type="number" className="search_input requerido" 
																		onChange={handleInputChange} 
																		onKeyDown={handleKeyDown} 
																		onKeyUp={handleKeyUp} 
															...
															<Col span={8}>
																<Form.Item name="CANT_VOLUMNE" label={<label style={{width:'80px'}}>Cant. Vol.</label>}>
																	<Input type="number" className="search_input requerido" 
																		onChange={handleInputChange} 
																		onKeyDown={handleKeyDown} 
																		onKeyUp={handleKeyUp} 
														...
														<Form.Item
															label={<label style={{width:'100px'}}>Tipo Compra</label>}
															name="COND_COMPRA"
															// onChange={async(e)=>{
															// 	modifico();
															// 	var row  = await grid.current.instance.getDataSource()._items[getFocusGlobalEvent().rowIndex]
												...
												<Col span={5}>
													<Card style={{paddingTop:'12px',paddingBottom:'5px', background:'#d50000', marginBottom:'5px'}}>
														<Form.Item name="ESTADO" valuePropName="checked"
															// onChange={ (e)=> handleCheckbox(e,["S","N"]) }
														>
															<Checkbox> Esta Activo </Checkbox>
														</Form.Item>
														<Form.Item name="IND_MANEJA_STOCK" valuePropName="checked"
															// onChange={ (e)=> handleCheckbox(e,["S","N"]) }
														>
															<Checkbox> Control de Stock	</Checkbox>
														</Form.Item>
														<Form.Item name="IND_MANEJA_VTO" valuePropName="checked"
															// onChange={ (e)=> handleCheckbox(e,["S","N"]) }
														>
															<Checkbox> Control de Vencimiento </Checkbox>
														</Form.Item>
														<Form.Item name="IND_COND_VTA" valuePropName="checked"
															// onChange={ (e)=> handleCheckbox(e,["S","N"]) }
														>
															<Checkbox> Condicionar Venta </Checkbox>
														</Form.Item>
														<Form.Item name="IND_INPASA" valuePropName="checked"
															// onChange={ (e)=> handleCheckbox(e,["S","N"]) }
														>
															<Checkbox> Ventas por INPASA </Checkbox>
														</Form.Item>
														<Form.Item name="IND_ESPECIAL" valuePropName="checked"
															// onChange={ (e)=> handleCheckbox(e,["S","N"]) }
														>
															<Checkbox> Producto Especial </Checkbox>
														</Form.Item>
														<Form.Item
															name="IND_JNJ"
															...
															// onChange={ (e)=> handleCheckbox(e,["S","N"]) }
														>
															<Checkbox> Producto JNJ </Checkbox>
														</Form.Item>
														<Form.Item
															name="ART_ADICIONAL"
															valuePropName="checked"
															// onChange={ (e)=> handleCheckbox(e,["S","N"]) }
														>
															<Checkbox> Tiene articulo Adicional </Checkbox>
														</Form.Item>
														<Form.Item name="ES_PESABLE" valuePropName="checked"
															// onChange={ (e)=> handleCheckbox(e,["S","N"]) }
														>
															<Checkbox> Es Pesable </Checkbox>
														</Form.Item>
													</Card>
