...
const Reporte = ()=>{
    ...
    const getData = async() =>{
        ...
        await Main.Request( url, 'POST',{tipo:'tienda_descuento',cod_empresa: sessionStorage.getItem('cod_empresa')})
            .then( response =>{
    ...
    const Build = async(rows_data) =>{
        ...
        for (let index = 0; index < rows_data.length; index++) {
            ...
            html += '<td rowSpan="' + len + '">' + await getInfo(info, "COD_CATEGORIA"   , "DESC_CATEGORIA") + '</td>';
            html += '<td rowSpan="' + len + '">' + await getInfo(info, "COD_SEGMENTO"    , "DESC_SEGMENTO") + '</td>';
            html += '<td rowSpan="' + len + '">' + item.COD_VISITA + '</td>';
            if(len == 1){
                html += productos;  
                html += '</tr>';
            }
        ...
        $('#table-body-tienda-descuento').empty();
        $('#table-body-tienda-descuento').append(html);
    }
    ...
    const export2PDF = async() =>{
        ...
        pdfDoc.autoTable({
            ...
            columnStyles:{
                ...
                // Descripcion
                14: {halign: 'left', cellWidth:100},
                // Unidad de medida
                15: {halign: 'left', cellWidth:40},
                // Cantidad
                16: {halign: 'right', cellWidth:30},
                // Porcentaje
                17: {halign: 'right', cellWidth:40},
            },
            ...
            didDrawPage: function (data) {
                ...
                pdfDoc.text('Tácticos - Tienda Descuento',(pdfDoc.internal.pageSize.getWidth() / 2) - 70,30);
        ...
        pdfDoc.save('tacticos_tienda_descuento_' + moment().format('DD_MM_YYYY') + '.pdf');
    }

    const export2Excel = async() =>{
        setActivarSpinner(true);
        createHtml2ExportTable2Excel2(data,'Tactico_tienda_descuento_' + moment().format('DD_MM_YYYY') );
        setTimeout( ()=> {
            setActivarSpinner(false);
        }, 200);
    }
            ...
            <table  className="ui basic compact celled table" 
                ...
                <thead className="headTable">
                    <tr>
                        ...
                        <th rowSpan="2" width="100">Categoria</th>
                        <th rowSpan="2" width="100">Segmento</th>
                        <th rowSpan="2" width="100">Calendario</th>
                ...
                <tbody id="table-body-tienda-descuento" className="table-prueba"></tbody>
            </table>
        </Spin>
    )
}
