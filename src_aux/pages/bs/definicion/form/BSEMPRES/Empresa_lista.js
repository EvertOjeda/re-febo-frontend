...
const urlBuscador       = "/bs/empresas/search";
const columBuscador     = 'DESCRIPCION';
const doNotsearch       = ['ACCION'];
const notOrderByAccion  = ['ACCION'];
const idACCION          = 'ACCIONES';  
const columns = [
    { ID: "COD_EMPRESA"     , label: "Codigo"       , width:100    },
    { ID: "DESCRIPCION"     , label: "Descrición"   , maxWidth:200 },
    { ID: "RUC_EMPRESA"     , label: "R.U.C"        , maxWidth:200 },
    { ID: "DIRECCION"       , label: "Dirección"    , minWidth:150 },
];
const defaultOpenKeys     = sessionStorage.getItem("mode") == 'vertical' ? [] : ['BS','BS-BS1'];
const defaultSelectedKeys = sessionStorage.getItem("mode") == 'vertical' ? [] : ['BS-BS1-null-BSEMPRES'];
const FormName            = 'BSEMPRES';
export default () => {
    const url_base      = "/bs/empresas"
    ...
    const handleDelete = async() => {
        ...
        data[0]['COD_EMPRESA_AUD']  = cod_empresa;
