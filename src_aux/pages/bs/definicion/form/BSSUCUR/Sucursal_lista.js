...
const columns = [
    ...
    { ID: 'DESCRIPCION'     , label: 'Descricion' , minWidth: 150 },
    { ID: 'DETALLE_DIR_RRHH', label: 'Dirección'  , minWidth: 370 },
    { ID: 'ES_MATRIZ'       , label: 'Matriz'     , minWidth: 30, align:'left', checkbox:true, options:["S"]},
    { ID: 'ESTADO'          , label: 'Estado'     , minWidth: 30, align:'left', checkbox:true, options:["A"]}
];
const defaultOpenKeys     = sessionStorage.getItem("mode") == 'vertical' ? [] : ['BS','BS-BS1'];
const defaultSelectedKeys = sessionStorage.getItem("mode") == 'vertical' ? [] : ['BS-BS1-null-BSSUCUR'];
const FormName            = 'BSSUCUR';
export default () =>{
    const url_base      = "/bs/sucursales/" + sessionStorage.getItem('cod_empresa');
    const urlBuscador   = "/bs/sucursales/search/" + sessionStorage.getItem('cod_empresa');
        ...
        await Main.Request( url, method, data[0]).then(async( response ) => {
            if(response.status == 200){
                if(response.data.ret == 1) {
                    ...
                    message.success({ content: 'Procesado correctamente', updatable:"updatable", duration: 5, style: {
                        marginTop: '10vh',
                    }});
                }else{
                    handleCancel();
